...
def dag_test_ikorchagin():
        ...
        query = '''SELECT  event_date,
                    ...
                    FROM
                        (SELECT toDate(time) event_date,  
                         ...
                         WHERE event_date = today() - 1
                         GROUP BY user_id, event_date) table1
                         LEFT JOIN
                        (SELECT toDate(time) event_date,  
                                reciever_id,
                         ...
                         GROUP BY reciever_id, event_date) table2
                         ON table1.user_id = table2.reciever_id
                        '''
    ...
    def load(final_df):
        q = '''
                        CREATE TABLE IF NOT EXISTS test.ikorchagin
                        (   event_date Date,
                            metric String,
                            metric_value String,
                            views UInt64,
        ...
        pandahouse.to_clickhouse(df=final_df, table='ikorchagin', index=False, connection=connection_test)
...
dag_test_ikorchagin = dag_test_ikorchagin()
